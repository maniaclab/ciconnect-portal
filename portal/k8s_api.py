...
templates = Environment(
    loader=FileSystemLoader("portal/yaml"), autoescape=select_autoescape()
)
...
def load_kube_config():
    try:
        ...
        logger.info(
            "GPU is available as a resource"
            if gpu_available
            else "GPU is not available as a resource"
        )
    ...
    while True:
        ...
        for pod in pods:
            if has_notebook_expired(pod):
                try:
                    logger.info(
                        "Notebook %s in namespace %s has expired"
                        % (pod.metadata.name, namespace)
                    )
                    remove_notebook(pod.metadata.name)
                except:
                    logger.info(
                        "Error removing notebook %s during management cycle"
                        % pod.metadata.name
                    )
        time.sleep(1800)
...
def create_service(notebook_id, image):
    try:
        ...
        service = yaml.safe_load(
            template.render(namespace=namespace, notebook_id=notebook_id, image=image)
        )
...
def notebook_id_available(notebook_id):
    try:
        ...
        pods = core_v1_api.list_namespaced_pod(
            namespace, label_selector="instance={0}".format(notebook_id)
        )
        ...
        logger.error(
            "Error checking whether notebook name %s is available" % notebook_id
        )
        raise k8sException(
            "Error checking whether notebook name %s is available" % notebook_id
        )
...
def validate(
    ...
    if not memory_request_valid(memory):
        ...
        return k8sException(
            "The request of %d GB is outside the bounds [1, 16]" % memory
        )
...
def get_expiration_date(pod):
    try:
        ...
        logger.error(
            "Error getting expiration date for notebook %s in namespace %s"
            % (pod.metadata.name, namespace)
        )
...
def notebook_closing(pod):
    try:
        ...
        logger.error(
            "Error checking whether notebook is closing in pod %s" % pod.metadata.name
        )
...
def get_notebook_status(pod):
    try:
        ...
        if notebook_closing(pod):
            ...
            core_v1_api = client.CoreV1Api()
            log = core_v1_api.read_namespaced_pod_log(
                pod.metadata.name, namespace=namespace
            )
            if re.search("Jupyter Notebook.*is running at.*", log) or re.search(
                "Jupyter Server.*is running at.*", log
            ):
                return "Ready"
            else:
...
def get_display_name(pod):
    try:
        ...
        logger.error(
            "Error getting value for display-name in pod %s" % pod.metadata.name
        )
...
def remove_user_notebook(notebook_name, username):
    try:
        ...
        if pod.metadata.labels["owner"] == username:
            ...
            logger.warning(
                "Notebook %s does not belong to user %s" % (notebook_id, username)
            )
            raise k8sException(
                "Notebook %s does not belong to user %s" % (notebook_id, username)
            )
